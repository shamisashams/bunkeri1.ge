...
export const RoundButton = ({ link, text, white }) => {
    return (
        <Link href={link}>
            <button
                className={
                    white ? "round_btn white archy-edt" : "round_btn archy-edt"
                }
            >
                {text}
            </button>
...
export const AddToCart = ({ onClick }) => {
    const sharedData = usePage().props.localizations;
    return (
        <button onClick={onClick} className="add_to_cart">
            <Cart color="#303285" />
...
export const YellowButton = ({ text, onclick = null }) => {
    return (
        <button onClick={onclick} className="yellow_button archy-edt">
            {text}
        </button>
    );
};
