        ...
        options.map((item, index) => {
            ...
            rows.push(
                <div className="flex" key={index}>
                    ...
                    }} name={ code } type="checkbox" id={`${code}-${index}`} value={item.id} checked={checked}/>
            ...
            return (
                <div className="section">
                    <div className="head">{item.name}</div>
                        {item.type !== 'boolean' ? options(item.code,item.options) : <input style={{display:"block"}} className="filter_ckbox" onClick={(event) => {
                            handleFilterClick(event,item.code,item.id)
                        }} name={ item.code } type="checkbox" id={`${item.code}`} value="1" checked={checked}/>}
                </div>
                )
        })}
    </div>
